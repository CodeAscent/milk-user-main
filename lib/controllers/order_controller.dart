...
//  PayLabPaymentController payLabPaymentController = PayLabPaymentController();
  ...
  void orderPlaceApi(BuildContext context, String method, int timeSlot) async {
    ...
    Map<String, dynamic> _map = {
      ...
      "coupon_code":
          appState.offerItem == null ? "" : appState.offerItem!.couponName,
      "is_coupon_applied": appState.offerItem == null ? 0 : 1,
      "promotional_disount": appState.offerItem == null
          ? 0.0
          : appState
              .offerItem!.discountPrice, //TODO: discount calculate from coupon
      "subtotal": appState.subTotal,
      "tax": appState.defaultTax, // Tax %//TODO: improve calculation
      "delivery_fee": appState.shippingCharge,
      "final_amount": appState
          .finalTotal, // Final Amount == Sub Total + Dliver + Tax - Discount
    ...
    await orderPlace(_map).then((value) {
      if (value.success!) {
        ...
        commonAlertNotification("Success",
            message:
                UtilsHelper.getString(context, "order_place_successfully"));
        Navigator.of(context).pushReplacementNamed(RoutePath.order_placed,
            arguments: value.data['id'].toString());
      } else {
  ...
  void getOrderListApi() async {
    getOrderList().then((value) {
      if (value.data != null) {
        orderListItems = List<OrderListItem>.from(
            value.data.map((x) => OrderListItem.fromJson(x)));
      }
      setState(() {});
    })
        //     .catchError((e) {
  ...
  Future markProductVacation({required String deliveryProductId}) async {
    try {
      final res = await markVacation(deliveryProductId: deliveryProductId);
      CommonResponse commonResponse = CommonResponse.fromJson(res);
      commonAlertNotification("Error",
